...
class PythonSourceGeneratorTransformer(ast.NodeTransformer):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == "EventDataset":
            ...
            if len(node.args) >= 1:
                if hasattr(node.args[0], "elts"):
                    ...
                    urls = [node.args[0]]
                paths = [
                    "".join(urlparse(ast.literal_eval(url))[1:])
                    for url in urls
                    if ast.literal_eval(url) is not None
                ]
                source_rep = (
    ...
    def visit_Select(self, node):
        ...
        node.rep = (
            "(lambda selection: ak.zip(selection, depth_limit="
            + repr(self._depth)
            + ")"
            + " if not isinstance(selection, ak.Array)"
            + " else selection)("
            + self.get_rep(call_node)
            + ")"
        )
        self._depth -= 1
        return node
    ...
    def visit_SelectMany(self, node):
        if type(node.selector) is not ast.Lambda:
            raise TypeError(
                "Argument to SelectMany() must be a lambda function, found "
                + node.selector
            )
        if len(node.selector.args.args) != 1:
            raise TypeError(
    ...
    def visit_Count(self, node):
        self.visit(node.source)
        node.rep = (
            "ak.num(" + self.get_rep(node.source) + ", axis=" + repr(self._depth) + ")"
        )
        return node
